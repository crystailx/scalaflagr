...
object Main {
  ...
  def main(args: Array[String]): Unit = {
    ...
    lazy val cacher = new RedisCache(scredis.Redis.apply("localhost", 36379, Some(AuthConfig(None, "test"))))
    lazy val service = new FlagrService(client, cacher)

    def createFlag: Future[Flag] = manager.createFlag(createFlagRequest description "test flag" key "test-flag")

    def addVariant(flagID: Long): Future[Variant] =
      manager.createVariant(flagID, createVariantRequest key "var1" attachment Attachment("homepage"))
    ...
    def setDistribution(flagID: Long, segmentID: Long, variantKey: String, variantID: Long): Future[List[Distribution]] =
      manager.updateDistributions(flagID, segmentID, updateDistributionsRequest distributions List(
