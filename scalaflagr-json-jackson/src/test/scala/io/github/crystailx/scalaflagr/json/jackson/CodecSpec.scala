...
class CodecSpec extends AnyFlatSpec with Matchers {
  ...
  it must "test jackson" in {
    val decoded = mapper.readValue[Flag](flagJson)
  ...
  it must "encode protected fields" in {
    val variant = Variant(
      key = "vKey",
      attachment = Some(implicitly[Encoder[Attachment]].encode(Attachment(Map("wow" -> "cool"))))
    )
    val encoded = new String(implicitly[Encoder[Variant]].encode(variant))
    println(encoded)
