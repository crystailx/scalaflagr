...
sealed trait Response[+U] {
  ...
  final def fold[V](ifError: => V)(f: U => V): V =
    if (isError) ifError else f(result)
  ...
  final def toEither: Either[FlagrError, U] =
    if (isError) Left(error) else Right(result)
}
...
case class RequestSuccess[U](
  ...
  override def error =
    throw new NoSuchElementException(s"Request success $result")

  final def map[V](f: U => V): Response[V] =
    RequestSuccess(status, body, headers, f(result))
...
case class RequestFailure(
    ...
    override val body: Option[String], // the http response body if the response included one
    override val headers: Map[String, String], // any http headers included in the response
    override val error: FlagrError
) extends Response[Nothing] {
  override def result =
    throw new NoSuchElementException(s"Request Failure $error")
